...
class _PlanTabViewState extends ConsumerState<PlanTabView>
  ...
  /// 今日と同じ日付の Day の index を返す (旅行期間外なら 0)。
  ...
  void _syncController(List<Day> days) {
    if (_controller == null || _controller!.length != days.length) {
      // 初回は「今日の Day」を初期選択、再生成時 (期間編集で Day 数が変わった等)
      // はユーザーの選択位置を clamp して引き継ぐ (毎回今日に飛ばさない)。
          ...
          children: [
            Material(
              ...
              child: TabBar(
                ...
                tabs: [
                  // 「Day N」 + 日付の 2 段表記。1 行だと横に伸びすぎて
                  // スクロール量が増えるため 2 段にする (既定高 46 では窮屈)。
                  for (final d in days)
                    Tab(
                      height: 54,
                      child: Column(
